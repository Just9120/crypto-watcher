...
CryptoWatcher в текущем виде — это **Bybit Spot Volume Radar**, а не общий мониторинг цены и не трекер портфеля.
...
- настраиваемые пороги сигнала на уровне чата
...
- preview алерта через `/sample_alert`
...
- **Спайк объёма** — отношение объёма последней закрытой свечи к среднему объёму предыдущих свечей
...
В `/settings` секции идут в порядке:
1. `Проверка рынка`
2. `Таймфрейм сигнала`
3. `Движение цены`
4. `Спайк объёма`
...
Кнопки действий внизу настроек:
- `📊 Статус`
- `🔄 Обновить`
...
- `/terms` — показать краткое объяснение логики сигнала
- `/sample_alert` — показать пример алерта
...
`/status` рендерится с учётом активных per-chat настроек:
- показывает активные `signal_timeframe`, `price_move_min`, `turnover_spike_min`, `liquidity_floor_24h` в хедере
- по каждому символу показывает текущие значения для `Движение цены`, `Спайк объёма`, `Ликвидность 24ч`
- по каждому фильтру показывает pass/fail относительно текущих порогов чата
- завершает блок символа итогом: `Итог: сигнал есть` или `Итог: сигнала нет`
...
Важно:
- `.env` задаёт **дефолты**
- если пользователь меняет timeframe сигнала или пороги сигнала в боте, активными становятся **значения из chat state**
