...
1. `Движение цены` ≥ порога чата
2. `Спайк объёма` ≥ порога чата
3. `Ликвидность 24ч` ≥ порога чата
...
Важно:
- пороги сигнала настраиваются в Telegram-кнопках на уровне чата
- значения из `.env` (`PRICE_MOVE_MIN`, `TURNOVER_SPIKE_MIN`, `LIQUIDITY_FLOOR_24H`) используются как дефолты для новых чатов и fallback для старых state
