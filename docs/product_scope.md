...
- настраиваемые на уровне чата пороги сигнала:
  - `Движение цены`
  - `Спайк объёма`
  - `Ликвидность 24ч`
...
- preview алерта внутри Telegram
- персистентный state на уровне чата для режима радара, пользовательского списка, timeframe сигнала, порогов сигнала, mute-состояния и runtime workflow
...
Пользовательские названия этих трёх фильтров:
...
- **Движение цены** — изменение цены на закрытой свече выбранного timeframe
- **Спайк объёма** — отношение объёма последней закрытой свечи к среднему объёму предыдущих свечей выбранного timeframe
- **Ликвидность 24ч** — 24h оборот пары на Bybit
...
- default timeframe сигнала = `5m`
- пороги сигнала настраиваются на уровне чата
- `.env` задаёт дефолтные значения, а не должен считаться единственным пользовательским способом управления порогами
...
- UI должен правдиво показывать:
  ...
  - radar poll
  - активные пороги сигнала
- в `/settings` секции должны идти в порядке:
  1) `Проверка рынка`
  2) `Таймфрейм сигнала`
  3) `Движение цены`
  4) `Спайк объёма`
  5) `Ликвидность 24ч`
- action-кнопки в settings: `📊 Статус`, `🔄 Обновить`
- пользовательские названия должны быть человеческими и русскими, а не code-like
...
Для новых per-chat порогов сигнала:
- старые чаты без этих полей должны безопасно получать env-backed дефолты
...
- выбранные пороги сигнала правдиво отражаются в UI и реально используются в расчётах
