...
При расхождении формулировок между документами и кодом действует порядок:
...
Отдельно: `.env.example` поля `BYBIT_PAIRS` и `WATCHLIST` считаются legacy/default seed-значениями для bootstrap/backward compatibility, а не основным механизмом ведения пользовательского списка.
...
Поле `baselines` в state допустимо как runtime/операционное и legacy-compatible хранилище. Это не означает возврат к baseline-driven модели как к основному продукту.
